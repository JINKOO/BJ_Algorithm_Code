/*
   ...
   #. 문제
	  ...
	  마지막으로, P1 = 3, P2 = 5, P3 = 15, P4 = 16인 경우에는 3개 들어있는 카드팩과 1개 들어있는 카드팩을 구매해 18원을 
	  지불하는 것이 최댓값이다.
...
/*
   #. 나의 풀이법
      - 카드 개수 만큼 dp[]를 생성하고,
		...
		카드 개수가 n일 때, 만들 수 있는 가지 수
		1, n-1
		2, n-2
		3, n-3
		... 이 중 최댓값이 dp[i]이다.

		2번 카드팩에는 2장의 카드팩이 가격 5이다.
		n=2일 때, dp[2] = max(cost[2], dp[1] + cost[1])이 된다.

		n=3일 때, dp[3] = max(cost[3], dp[2] + cost[1])
		                               dp[1] + cost[2])

		즉, dp[i] = max(cost[i], dp[i-j] + cost[j]);
		어느 경우가 최대인지 각 카드팩에서 따져야 하므로, 이중 반복문 필요하다.
*/
#include <iostream>
using namespace std;
